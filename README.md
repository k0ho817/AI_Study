...
s = svm.SVC(gamma=0.1, C=10)
```
위의 라인에서 sklearn 라이브러리는 SVM을 위해 분류 모델 SVC와 회귀 모델 SVR을 제공하는데, 여기서는 분류 모델 SVC를 호출한다. 커널 함수를 지정하는 매개변수는 kernel인데 생략했기 때문에 기본값인 rbf(radial basis function)를 사용한다. 매개변수 gamma는 커널 함수와 관련된 계수이다.
